...
<div class="page-title">
  ...
  <div class="container padding-bottom-3x mb-1">
                ...
                <div class="shop-top-filter-wrapper">
                    <div class="row">
                        ...
                        <div class="col-md-2 gd-text-sm-center">
                            {{-- Theme CSS expects .grid-view / .list-view (not both .list-view) for layout + indicators --}}
                            <div class="shop-view">
                                <a class="grid-view {{ Session::get('view_catalog', 'grid') === 'grid' ? 'active' : '' }}" data-step="grid" href="javascript:;" data-href="{{ route('front.catalog').'?view_check=grid' }}" title="{{ __('Grid view') }}"><i class="fas fa-th-large" aria-hidden="true"></i></a>
                                <a class="list-view {{ Session::get('view_catalog', 'grid') === 'list' ? 'active' : '' }}" href="javascript:;" data-step="list" data-href="{{ route('front.catalog').'?view_check=list' }}" title="{{ __('List view') }}"><i class="fas fa-list" aria-hidden="true"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="row g-3">
          ...
          <div class="col-lg-3 order-lg-1">
            ...
            <aside class="sidebar sidebar-offcanvas position-left"><span class="sidebar-close"><i class="icon-x"></i></span>
              ...
              @if (! empty($facet_metal_types))
              <section class="widget widget-categories card rounded p-4">
                <h3 class="widget-title">{{ __('Filter by Metal Type') }}</h3>
                @foreach ($facet_metal_types as $val)
                  @php $id = 'mt_' . preg_replace('/[^a-zA-Z0-9_-]+/', '_', strtolower($val)); @endphp
                  <div class="custom-control custom-checkbox">
                    <input class="custom-control-input csv-filter csv-filter--metal-type" type="checkbox"
                      value="{{ $val }}" id="{{ $id }}"
                      {{ in_array($val, array_filter(array_map('trim', explode(',', (string) request()->get('metal_type'))))) ? 'checked' : '' }}>
              ...
              {{-- Brand filter removed for jewelry-focused catalog --}}
      ...
      <form id="search_form" class="d-none" action="{{route('front.catalog')}}" method="GET">
        ...
        <input type="text" name="metal_type" id="metal_type" value="{{ request()->get('metal_type', '') }}">
...
@endsection
<script>
   function syncCsvFilter(selector, hiddenInputId) {
            var vals = [];
            $(selector).each(function () {
                if ($(this).is(':checked')) {
                    vals.push(String($(this).val()).trim());
                }
            });
            $("#search_form #" + hiddenInputId).val(vals.join(','));
        }

        $(document).on('click', '.csv-filter--metal-type', function () {
            syncCsvFilter('.csv-filter--metal-type', 'metal_type');
            removePage();
            $("#search_button").click();
        });

        $(document).on('click', '.csv-filter--diamond-quality', function () {
            syncCsvFilter('.csv-filter--diamond-quality', 'diamond_quality');
            removePage();
            $("#search_button").click();
        });

        $(document).on('click', '.csv-filter--diamond-color', function () {
            syncCsvFilter('.csv-filter--diamond-color', 'diamond_color');
            removePage();
            $("#search_button").click();
        });
</script>
