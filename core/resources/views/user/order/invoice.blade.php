                                ...
                                @foreach (json_decode($order->cart, true) as $key => $item)
                                    ...
                                    <tr>
                                        <td class="">
                                            {{ $item['name'] }}
                                            @php
                                                $invShape = $item['selected_shape'] ?? null;
                                                $invCarat = $item['selected_carat'] ?? ($item['carat_weight'] ?? null);
                                                $invMetal = trim((string) ($item['metal_type'] ?? $item['pdp_metal_type'] ?? ''));
                                                $invKarat = trim((string) ($item['gold_karat'] ?? $item['pdp_gold_karat'] ?? ''));
                                                $invMetalLine = trim($invMetal . ($invKarat !== '' ? ' ' . $invKarat : ''));
                                            @endphp
                                            @if ($invShape || $invCarat || $invMetalLine !== '')
                                                <div class="small text-muted mt-1">
                                                    @if ($invShape)<span>{{ $invShape }}</span>@endif
                                                    @if ($invCarat)<span>@if ($invShape) · @endif{{ $invCarat }} CT</span>@endif
                                                    @if ($invMetalLine !== '')<span>@if ($invShape || $invCarat) · @endif{{ $invMetalLine }}</span>@endif
                                                </div>
                                            @endif
                                            <p>
                                                @if ($main_item)
                                        ...
                                        <td class="px-0">
                                            ...
                                            @if (! empty($item['engraving']))
                                                <div>{{ __('Engraving') }}: {{ $item['engraving'] }}</div>
                                            @endif
                                            @if (! empty($item['ring_size']))
                                                <div>{{ __('Ring size') }}: {{ $item['ring_size'] }}</div>
                                            @endif
                                            @if (! empty($item['metal_type'] ?? $item['pdp_metal_type'] ?? null))
                                                <div>{{ __('Metal type') }}: {{ $item['metal_type'] ?? $item['pdp_metal_type'] }}</div>
                                            @endif
                                            @if (! empty($item['gold_karat'] ?? $item['pdp_gold_karat'] ?? null))
                                                <div>{{ __('Gold karat') }}: {{ $item['gold_karat'] ?? $item['pdp_gold_karat'] }}</div>
                                            @endif
                                            @if (! empty($item['color_grade'] ?? $item['pdp_diamond_color'] ?? null))
                                                <div>{{ __('Diamond colour') }}: {{ $item['color_grade'] ?? $item['pdp_diamond_color'] }}</div>
                                            @endif
