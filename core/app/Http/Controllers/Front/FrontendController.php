...
class FrontendController extends Controller
{
    ...
    public function product($slug)
    {
        ...
        $item = Item::with(['category', 'diamondAttribute', 'galleries'])->whereStatus(1)->whereSlug($slug)->firstOrFail();
        $video = explode('=', $item->video);

        $defaultPdpUrls = collect([\App\Helpers\ImageHelper::storageImageUrl($item->photo)]);
        foreach ($item->galleries as $g) {
            $defaultPdpUrls->push(\App\Helpers\ImageHelper::storageImageUrl($g->photo));
        }
        $defaultPdpUrls = $defaultPdpUrls->filter()->unique()->values()->all();

        $metalTypes = \App\Models\Item::normalizeJewelryOptionList($item->metal_type) ?? [];
        $initialMetal = (string) ($metalTypes[0] ?? '');
        if ($initialMetal === '') {
            $metalFirstRow = collect($item->pdp_metal_variants ?? [])->first(function ($v) {
                return is_array($v) && (! empty($v['image']) || ! empty($v['photo']));
            });
            $initialMetal = (string) ($metalFirstRow['label'] ?? $metalFirstRow['key'] ?? '');
        }

        $da = $item->diamondAttribute;
        $shapeList = \App\Models\Item::normalizeJewelryOptionList($da?->shape ?? null) ?? [];
        $initialShape = (string) ($shapeList[0] ?? '');

        $pdp_slider_images = $item->resolvePdpGalleryUrls($initialShape, $initialMetal, $defaultPdpUrls);
        if ($pdp_slider_images === []) {
            $pdp_slider_images = $defaultPdpUrls;
        }

        $pdp_primary_still = $pdp_slider_images[0] ?? null;

        $complete_the_look_items = $item->completeTheLookItems();
