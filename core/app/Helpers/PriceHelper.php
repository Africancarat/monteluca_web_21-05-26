...
class PriceHelper
{
    ...
    public static function stockDecrese(?array $cart = null)
    {
        ...
        foreach ($cart as $key => $item) {
            $itemId = (int) self::GetItemId((string) $key);
            $main_item = Item::find($itemId);
            if (! $main_item || $main_item->item_type != 'normal') {
                continue;
            }

            $qty = (int) ($item['qty'] ?? 1);
            $current = (int) $main_item->stock - $qty;
            $main_item->stock = $current <= 0 ? 0 : $current;
            $main_item->save();

            $optionIds = $item['options_id'] ?? [];
            ...
            foreach ($optionIds as $id) {
                ...
                $new_stock = (int) $option->stock - $qty;
                $option->stock = $new_stock <= 0 ? '0' : (string) $new_stock;
                $option->save();
            }
        }
    }
    ...
    /**
     * Restore physical stock from a cart snapshot (order cancel / refund).
     *
     * @param  array<string, mixed>|null  $cart
     */
    public static function restock(?array $cart = null): void
    {
        $cart = $cart ?? Session::get('cart');
        if (! is_iterable($cart)) {
            return;
        ...
        foreach ($cart as $key => $item) {
            ...
            $qty = (int) ($item['qty'] ?? 1);
            $main_item->stock = (int) $main_item->stock + $qty;
            $main_item->save();
            ...
            foreach ($optionIds as $id) {
                ...
                $option->stock = (string) ((int) $option->stock + $qty);
                $option->save();
            }
        }
    }
